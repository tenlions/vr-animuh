...
{
    public class FireBallPunch : MonoBehaviour
    {
        ...
        {
            if (!other.CompareTag("Target") || currentFireball2 == null) return;

            DebuggableScript.DebugLog(this.name, "collided with " +  other.name);
            Rigidbody fireBallRigidbody = currentFireball2.GetComponent<Rigidbody>();
            Vector3 localVelocity = fireBallRigidbody.GetPointVelocity(other.transform.position);
            Debug.DrawLine(localVelocity, localVelocity * 3, Color.yellow, 10f);
        }
